...
public interface GumshudaRepository extends JpaRepository<Gumshuda, Long> {
    ...
    @Query("SELECT g FROM Gumshuda g JOIN FETCH g.masjid WHERE g.masjid.id = :masjidId AND g.isDeleted = :isDeleted AND g.active = :active")
    List<Gumshuda> findByMasjidIdAndIsDeletedAndActive(@Param("masjidId") Long masjidId, @Param("isDeleted") Integer isDeleted, @Param("active") Boolean active);
